...
def get_unique_values(in_data, fields):
    ...
    arcpy.Statistics_analysis(in_data, table_name, [
                            (fields, 'FIRST')], fields)
...
def create_name(workspace, name, extension, prefix):
    """
    ...
    unique_name = arcpy.CreateUniqueName(
        prefix + validated_name, workspace)
...
def select(datatype, *args):
    ...
    feature_data = datatype in [
        'FeatureClass', 'FeatureLayer', 'ShapeFile']
    tool_name = 'Select' if feature_data else 'TableSelect'
    eval('arcpy.analysis.{}'.format(tool_name))(*args)

def split_by_atts(in_data, out_workspace, prefix):
    """
    ...
    for i in unique_values:
        output = create_name(out_workspace, i, extension, prefix)
    ...
    # try:
    #     split_by_atts(in_data, out_workspace, fields, prefix)
